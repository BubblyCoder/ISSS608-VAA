...
-   DEPENDENCY: the ratio between young and aged against economy active group \## Data wrangling The following data wrangling and transformation functions will be used:
...
```{r}
tmap_components <- data.frame(
Component = c("`tm_shape()`",
"`tm_fill()` or `tm_polygons()`",
"`tm_borders()`",
"`tm_layout()`",
"`tm_compass()` / `tm_scale_bar()`"),
`What it does` = c("Defines the spatial dataset you are using (the foundation).",
"Controls the actual data visualization (colors, classification breaks, palettes).",
"Customizes the look of the polygon boundaries (alpha, color, line weight).",
"Controls the 'canvas' aesthetics (legend position, frame, background color, fonts).",
"Adds essential map features for professional cartography.")
)
kable(tmap_components, format = "markdown")
```
...
map_jenks <- tm_shape(mpsz_pop2020) +
  tm_polygons("DEPENDENCY", fill.scale = tm_scale_intervals(style = "jenks", n = 5, values = "Blues")) +
  tm_borders(fill_alpha = 0.5) +
  tm_title("Jenks (Natural Breaks)")


map_equal <- tm_shape(mpsz_pop2020) +
  tm_polygons("DEPENDENCY", fill.scale = tm_scale_intervals(style = "equal", n = 5, values = "Blues")) +
  tm_borders(fill_alpha = 0.5) +
  tm_title("Equal Interval")


map_quantile <- tm_shape(mpsz_pop2020) +
  tm_polygons("DEPENDENCY", fill.scale = tm_scale_intervals(style = "quantile", n = 5, values = "Blues")) +
  tm_borders(fill_alpha = 0.5) +
  tm_title("Quantile")


map_sd <- tm_shape(mpsz_pop2020) +
  tm_polygons("DEPENDENCY", fill.scale = tm_scale_intervals(style = "sd", values = "Blues")) + 
  tm_borders(fill_alpha = 0.5) +  
...
# Map with 2 Classes
map_2 <- tm_shape(mpsz_pop2020) +
                                tm_polygons("DEPENDENCY",
                                            fill.scale = tm_scale_intervals(style = "jenks", n = 2)) +
                                tm_title("2 Classes")
                              # Map with 6 Classes
                              map_6 <- tm_shape(mpsz_pop2020) +
                                tm_polygons("DEPENDENCY",
                                            fill.scale = tm_scale_intervals(style = "jenks", n = 6)) +
                                tm_title("6 Classes")
                              # Map with 10 Classes
                              map_10 <- tm_shape(mpsz_pop2020) +
                                tm_polygons("DEPENDENCY",
                                            fill.scale = tm_scale_intervals(style = "jenks", n = 10)) +
                                tm_title("10 Classes")
                              # Map with 20 Classes
                              map_20 <- tm_shape(mpsz_pop2020) +
                                tm_polygons("DEPENDENCY",
                                            fill.scale = tm_scale_intervals(style = "jenks", n = 20)) +
                                tm_title("20 Classes")
                              # Arrange and explicitly print the final grid
                              tmap_arrange(list(map_2, map_6, map_10, map_20), ncols = 2)
```

::: callout-note
6 class appears to be the sweet spot. 2 class appears oversimplified 10 class appears to be difficult to differtentiate beyond 0.89 20 classes is too cluttered
:::

### Plotting choropleth map with custome break

or all the built-in styles, the category breaks are computed internally. In order to override these defaults, the breakpoints can be set explicitly by means of the breaks argument to the tm_fill(). It is important to note that, in tmap the breaks include a minimum and maximum. As a result, in order to end up with n categories, n+1 elements must be specified in the breaks option (the values must be in increasing order).
